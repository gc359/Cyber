...
caesar_cipher() {
  ...
  # Parameters:
  # $1 (str): The string to encrypt.
  # $2 (int): The number of positions to shift the characters.
  ...
  for (( i=0; i<${#text}; i++ )); do
    ...
    # If the character is a lowercase letter
      ...
      # Shift the character by the given amount and wrap around if necessary
      char_code=$(printf '%d' "'$char")
      shifted_code=$(((char_code - 97 + shift_amount) % 26 + 97))
      shifted_char=$(printf \\$(printf '%03o' $shifted_code))
      result+="$shifted_char"
...
echo "$encrypted_text"  # prints "Khoor, Zruog!"
